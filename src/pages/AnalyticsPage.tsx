...
export default function AnalyticsPage() {
  ...
  const stats = useMemo(() => {
    ...
    const totalCards = filteredCompletions.reduce((sum, c) => sum + c.summary.completedCards, 0);
    const totalTime = filteredCompletions.reduce((sum, c) => sum + c.summary.totalTimeSpent, 0);
    const avgCardsPerDay = totalDays > 0 ? (totalCards / totalDays).toFixed(1) : '0';
    ...
    return { totalDays, totalCards, totalTime, avgCardsPerDay, categoryTotals: categoryMap };
  }, [filteredCompletions]);
      ...
      <div className="flex-1 overflow-y-auto overscroll-contain" style={{ WebkitOverflowScrolling: 'touch' }}>
        ...
        {filteredCompletions.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-400 text-sm mt-2">Complete your first day to see analytics!</p>
          </div>
        ) : (
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-r from-orange-50 to-red-50 border border-orange-200 rounded-lg p-6">
              <div className="flex items-center justify-between">
                ...
                {dayCompletion.userStreak.longestStreak > dayCompletion.userStreak.currentStreak && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="bg-white border border-gray-200 rounded-lg p-6">
                <div className="text-sm text-gray-500 font-medium mb-1">Days Completed</div>
                <div className="text-3xl font-bold text-gray-900">{stats.totalDays}</div>
              </div>
              <div className="bg-white border border-gray-200 rounded-lg p-6">
                <div className="text-sm text-gray-500 font-medium mb-1">Total Cards Completed</div>
                <div className="text-3xl font-bold text-gray-900">{stats.totalCards}</div>
              </div>
              <div className="bg-white border border-gray-200 rounded-lg p-6">
                <div className="text-sm text-gray-500 font-medium mb-1">Total Time Spent</div>
                <div className="text-3xl font-bold text-gray-900">{formatDuration(stats.totalTime)}</div>
              </div>
              <div className="bg-white border border-gray-200 rounded-lg p-6">
                <div className="text-sm text-gray-500 font-medium mb-1">Avg Cards/Day</div>
                ...
                <div className="space-y-4">
                  {Array.from(stats.categoryTotals.entries())
                    ...
                    .map(([category, data]) => {
                      ...
                      const maxCount = Math.max(...Array.from(stats.categoryTotals.values()).map(v => v.count));
              ...
              <div className="bg-white border border-gray-200 rounded-lg p-6">
                ...
                <div className="space-y-3">
                  {recentCompletions.map(completion => (
                    ...
                    >
                      <div>
                        <div className="font-medium text-gray-900">{formatDate(completion.completedAt)}</div>
                        <div className="text-sm text-gray-600 mt-1">
                          {completion.summary.completedCards} of {completion.summary.totalCards} cards completed
                        </div>
                      </div>
                      <div className="text-right">
