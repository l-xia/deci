...
const Timer = forwardRef<TimerRef, TimerProps>(
  ({ card, onComplete, onTimerStateChange }, ref) => {
    ...
    return (
      <div className="mt-3 p-3 bg-white bg-opacity-50 rounded-md border border-gray-200">
        {!isRunning &&
          card.timerState?.isPaused &&
          card.timerState.accumulatedSeconds > 0 && (
            <div className="text-xs text-gray-500 mb-1">
              Paused at{' '}
              {formatTimerDuration(card.timerState.accumulatedSeconds)}
            </div>
          )}
